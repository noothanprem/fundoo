...
class TestNote:
     ...
     def test_note_get1(self):
        url = BASE_URL + "/note/noteupdate/"+(data[0]['noteget1']['note_id'])
        response = requests.get(url=url, headers=headers)
        assert response.status_code == 400

     def test_note_get2(self):
        url = BASE_URL + "/note/noteupdate/" + (data[0]['noteget2']['note_id'])
        response = requests.get(url=url, headers=headers)
        assert response.status_code == 400

     def test_note_get3(self):
         url = BASE_URL + "/note/noteupdate/" + (data[0]['noteget3']['note_id'])
         response = requests.get(url=url, headers=headers)
         assert response.status_code == 200

     def test_note_delete1(self):
        url = BASE_URL + "/note/noteupdate/" + (data[0]['notedelete1']['note_id'])
        response = requests.delete(url=url, headers=headers)
        assert response.status_code == 400

     def test_note_delete2(self):
         url = BASE_URL + "/note/noteupdate/" + str((data[0]['notedelete2']['note_id']))
         response = requests.delete(url=url, headers=headers)
         assert response.status_code == 400

     def test_note_delete3(self):
         url = BASE_URL + "/note/noteupdate/" + (data[0]['notedelete3']['note_id'])
         response = requests.delete(url=url, headers=headers)
         assert response.status_code == 200

     def test_note_put1(self):
         url = BASE_URL + "/note/noteupdate/" + (data[0]['noteput1']['note_id'])
         input = data[0]['notepost1']
         response = requests.delete(url=url, data=input, headers=headers)
         assert response.status_code == 400

     def test_note_put2(self):
         url = BASE_URL + "/note/noteupdate/" + (data[0]['noteput2']['note_id'])
         input=data[0]['notepost1']
         response = requests.delete(url=url, data=input,headers=headers)
         assert response.status_code == 400

     def test_note_put3(self):
         url = BASE_URL + "/note/noteupdate/" + (data[0]['noteput3']['note_id'])
         input = data[0]['notepost1']
         response = requests.delete(url=url, data=input, headers=headers)
         assert response.status_code == 200
...
class TestLabel:
    ...
    def test_label_put1(self):
        url = BASE_URL + "/note/updatelabel/"+(data[0]['labelget2']['label_id'])
